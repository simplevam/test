...
class GoodsModel extends Model{
    ...
    protected $_valid = array(
                            ...
                            array('shop_price',1,'本店售价不能为空','require'),
                            array('market_price',1,'市场售价不能为空','require'),
                            array('is_new',0,'is_new只能是0或1','in','0,1'),
                            array('is_hot',0,'is_hot只能是0或1','in','0,1'),
                            array('is_best',0,'is_best只能是0或1','in','0,1'),
                            array('goods_desc',1,'商品详细信息必须填写','require'),
                              array('goods_brief',1,'商品简介信息必须填写','require'),
                            array('goods_desc',2,'商品简介就在10到1000字符','length','10,1000')
    );


    /*
  is_delete = 1 即 商品放到回收站
    ...
    */
    ...
    public function createSn() {
        $sn = 'SP' . date('Ymd') . mt_rand(10000,99999);

        $sql = 'select count(*) from ' . $this->table . " where goods_sn='" . $sn . "'";

        return $this->db->getOne($sql)?$this->createSn():$sn;//查询到则调用自身再次生成 否者return $sn
    }
